...
module.exports.processCalendly = (event, context, callback) => {
  let calendlyInfo = JSON.parse(event.body);
  console.log(calendlyInfo);

  let slackWebhook = process.env.slackWebhook;
  let interviewTime = calendlyInfo.payload.event.invitee_start_time_pretty;
  let interviewStatus = calendlyInfo.event;
  let name = calendlyInfo.payload.invitee.name;
